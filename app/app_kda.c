...
static unsigned char *fixed_info_gen_concat(AMVP_KDA_PATTERN_CANDIDATE *fixedInfoPattern,
    ...
    for (i = 0; i < AMVP_KDA_PATTERN_MAX; i++) {
        if (fixedInfoPattern[i] == AMVP_KDA_PATTERN_L) {
            totalLen += 4;
            break;
        }
    }
...
int app_kda_hkdf_handler(AMVP_TEST_CASE *test_case) {
    ...
    if (stc->encoding != AMVP_KDA_ENCODING_CONCAT) {
...
int app_kda_onestep_handler(AMVP_TEST_CASE *test_case) {
    ...
    AMVP_SUB_HASH hashalg;
    AMVP_SUB_HMAC hmacalg;
    ...
    if (stc->aux_function == AMVP_KMAC_128) {
        mac = "KMAC128";
    } else if (!stc->salt) {
        hashalg = amvp_get_hash_alg(stc->aux_function);
        if (hashalg == 0) {
        ...
        hmacalg = amvp_get_hmac_alg(stc->aux_function);
        if (hmacalg == 0) {
            printf("Invalid cipher value");
            goto end;
        }
