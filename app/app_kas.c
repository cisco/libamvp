...
int app_kas_ecc_handler(AMVP_TEST_CASE *test_case) {
    ...
    if (tc->test_type == AMVP_KAS_ECC_TT_VAL) {
    ...
    if (tc->test_type == AMVP_KAS_ECC_TT_AFT) {
    ...
    if (tc->mode == AMVP_KAS_ECC_MODE_CDH) {
        tc->zlen = (int)z_len;
...
int app_kas_ffc_handler(AMVP_TEST_CASE *test_case) {
    ...
    if (tc->test_type == AMVP_KAS_FFC_TT_VAL) {
    ...
    if (tc->test_type == AMVP_KAS_FFC_TT_VAL || use_pqg) {
        if (EVP_PKEY_fromdata_init(pkey_ctx) != 1) {
    ...
    if (tc->test_type == AMVP_KAS_FFC_TT_AFT) {
        if (use_pqg) {
            if (pkey_ctx) EVP_PKEY_CTX_free(pkey_ctx);
            pkey_ctx = NULL;
...
int app_kas_ifc_handler(AMVP_TEST_CASE *test_case) {
    ...
    if (tc->kas_role == AMVP_KAS_IFC_INITIATOR || tc->scheme == AMVP_KAS_IFC_KAS2) {
    ...
    if (tc->kas_role == AMVP_KAS_IFC_RESPONDER || tc->scheme == AMVP_KAS_IFC_KAS2) {
        ...
        if (tc->key_gen == AMVP_KAS_IFC_RSAKPG1_CRT || tc->key_gen == AMVP_KAS_IFC_RSAKPG2_CRT) {
    ...
    if (!(tc->scheme == AMVP_KAS_IFC_KAS1 && tc->kas_role == AMVP_KAS_IFC_INITIATOR)) {
     ...
     */
    if (tc->kas_role == AMVP_KAS_IFC_INITIATOR) {
        /*
         ...
         */
        if (tc->test_type == AMVP_KAS_IFC_TT_VAL) {
        ...
        if (tc->scheme == AMVP_KAS_IFC_KAS2) {
            if (z) free(z);
            z = NULL;
        ...
        */
        ...
        if (tc->scheme == AMVP_KAS_IFC_KAS2 && tc->test_type == AMVP_KAS_IFC_TT_AFT) {
            if (z) free(z);
            z = NULL;
...
int app_kts_ifc_handler(AMVP_TEST_CASE *test_case) {
    ...
    if (tc->kts_role == AMVP_KTS_IFC_RESPONDER) {
        p = BN_bin2bn(tc->p, tc->plen, NULL);
        q = BN_bin2bn(tc->q, tc->qlen, NULL);
        if (!p || !q) {
        ...
        if (tc->key_gen == AMVP_KTS_IFC_RSAKPG1_CRT || tc->key_gen == AMVP_KTS_IFC_RSAKPG2_CRT) {
            dmp1 = BN_bin2bn(tc->dmp1, tc->dmp1_len, NULL);
            dmq1 = BN_bin2bn(tc->dmq1, tc->dmq1_len, NULL);
            iqmp = BN_bin2bn(tc->iqmp, tc->iqmp_len, NULL);
            if (!dmp1 || !dmq1 || !iqmp) {
    ...
    if (tc->kts_role == AMVP_KTS_IFC_INITIATOR) {
...
int app_safe_primes_handler(AMVP_TEST_CASE *test_case) {
    ...
    if (tc->cipher == AMVP_SAFE_PRIMES_KEYGEN) {
        params = OSSL_PARAM_BLD_to_param(pbld);
        if (!params) {
