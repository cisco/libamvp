...
int app_des_handler(AMVP_TEST_CASE *test_case) {
    ...
    alg = amvp_get_tdes_alg(tc->cipher);
    if (alg == 0) {
    ...
    default:
        printf("Error: Unsupported DES mode requested by AMVP server\n");
        goto err;
...
#define SYM_IV_BYTE_MAX 128
        ...
        if (tc->mct_index == AMVP_DES_MCT_INNER - 1) {
