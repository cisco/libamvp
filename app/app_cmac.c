...
int app_cmac_handler(AMVP_TEST_CASE *test_case) {
    ...
    alg = amvp_get_cmac_alg(tc->cipher);
    if (alg == 0) {
 ...
 switch (alg) {
    ...
    case AMVP_SUB_CMAC_TDES:
        ...
        break;
    default:
        printf("Error: Unsupported CMAC algorithm requested by AMVP server\n");
        return rv;
    }
...
#define CMAC_BUF_MAX 128
    ...
    if (tc->verify) {
        ...
        if (!diff) {
            tc->ver_disposition = AMVP_TEST_DISPOSITION_PASS;
        } else {
            tc->ver_disposition = AMVP_TEST_DISPOSITION_FAIL;
        }
    } else {
