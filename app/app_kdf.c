...
int app_kdf135_x942_handler(AMVP_TEST_CASE *test_case) {
    ...
    /**
     * For AMVP, we need to manually build the amvp-info string instead of using the regular param setters.
     * Format:
    ...
    if (info_len > 8) {
        amvp_info = calloc(info_len, sizeof(unsigned char));
        if (!amvp_info) {
            printf("Error allocating memory for amvp-info string in KDF x942\n");
            goto end;
        }
        amvp_info[iter] = 0xA0;
        iter++;
        amvp_info[iter] = (unsigned char)stc->party_u_len;
        iter++;
        memcpy_s(&amvp_info[iter], info_len - iter, stc->party_u_info, stc->party_u_len);
        iter += stc->party_u_len;
        amvp_info[iter] = 0xA1;
        iter++;
        amvp_info[iter] = (unsigned char)stc->party_v_len;
        iter++;
        memcpy_s(&amvp_info[iter], info_len - iter, stc->party_v_info, stc->party_v_len);
        iter += stc->party_v_len;
        amvp_info[iter] = 0xA2;
        iter++;
        amvp_info[iter] = (unsigned char)stc->supp_pub_len;
        iter++;
        memcpy_s(&amvp_info[iter], info_len - iter, stc->supp_pub_info, stc->supp_pub_len);
        iter += stc->supp_pub_len;
        amvp_info[iter] = 0xA3;
        iter++;
        amvp_info[iter] = (unsigned char)stc->supp_priv_len;
        iter++;
...
int app_kdf108_handler(AMVP_TEST_CASE *test_case) {
    ...
    case AMVP_KDF108_MAC_MODE_CMAC_AES256:
        ...
        isHmac = 0;
        break;
    case AMVP_KDF108_MAC_MODE_MIN:
    case AMVP_KDF108_MAC_MODE_CMAC_TDES:
    case AMVP_KDF108_MAC_MODE_MAX:
    default:
    ...
    memcpy_s(stc->fixed_data, AMVP_KDF108_FIXED_DATA_MAX, fixed, fixed_len);
    ...
    if (stc->mode == AMVP_KDF108_MODE_COUNTER) {
        OSSL_PARAM_BLD_push_utf8_string(pbld, OSSL_KDF_PARAM_MODE, "COUNTER", 0);
    } else if (stc->mode == AMVP_KDF108_MODE_FEEDBACK) {
...
int app_pbkdf_handler(AMVP_TEST_CASE *test_case) {
    ...
    OSSL_PARAM_BLD_push_int(pbld, OSSL_KDF_PARAM_PKCS5, 1); /* disables compliance checks, dont want limit checks for AMVP tests */
    params = OSSL_PARAM_BLD_to_param(pbld);
    if (!params) {
...
int app_kdf_tls13_handler(AMVP_TEST_CASE *test_case) {
    ...
    if ((tc->running_mode == AMVP_KDF_TLS13_RUN_MODE_PSK_DHE) ||
        (tc->running_mode == AMVP_KDF_TLS13_RUN_MODE_PSK)) {
    ...
    if ((tc->running_mode == AMVP_KDF_TLS13_RUN_MODE_PSK_DHE) ||
        (tc->running_mode == AMVP_KDF_TLS13_RUN_MODE_DHE)) {
