...
int app_kmac_handler(AMVP_TEST_CASE *test_case) {
    ...
    alg = amvp_get_kmac_alg(tc->cipher);
    if (alg == 0) {
    ...
    case AMVP_SUB_KMAC_256:
        ...
        break;
    default:
        printf("Error: Unsupported KMAC algorithm requested by AMVP server\n");
        return rv;
    }
...
#define KMAC_BUF_MAX 8192
    ...
    if (tc->test_type == AMVP_KMAC_TEST_TYPE_MVT) {
        ...
        if (!diff) {
            tc->disposition = AMVP_TEST_DISPOSITION_PASS;
        } else {
            tc->disposition = AMVP_TEST_DISPOSITION_FAIL;
        }
    } else {
