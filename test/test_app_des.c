...
AMVP_SYM_CIPHER_TC *des_tc;
AMVP_RESULT rv;

int initialize_des_tc(AMVP_SYM_CIPHER_TC *des_tc, int alg_id, char *pt, 
                      int pt_len, char *ct, int ct_len, char *key, int key_len,
                      char *iv, int iv_len, AMVP_SYM_CIPH_DIR direction, 
                      AMVP_SYM_CIPH_TESTTYPE test_type) {

    des_tc->ct = calloc(AMVP_SYM_CT_BYTE_MAX, sizeof(char));
    if (!des_tc->ct) { return 0; }
    if (ct) {
        rv = amvp_hexstr_to_bin(ct, des_tc->ct, AMVP_SYM_CT_BYTE_MAX, NULL);
        if (rv != AMVP_SUCCESS) {
    ...
    des_tc->pt = calloc(AMVP_SYM_PT_BYTE_MAX, sizeof(char));
    if (!des_tc->pt) { return -1; }
    if (pt) {
        rv = amvp_hexstr_to_bin(pt, des_tc->pt, AMVP_SYM_PT_BYTE_MAX, NULL);
        if (rv != AMVP_SUCCESS) {
            AMVP_LOG_ERR("Hex converstion failure (pt)");
            return 0;
        }
    }
    des_tc->key = calloc(AMVP_SYM_KEY_MAX_BYTES, sizeof(char));
    if (!des_tc->key) { return -1; }
    if (key) {
        rv = amvp_hexstr_to_bin(key, des_tc->key, AMVP_SYM_KEY_MAX_BYTES, NULL);
        if (rv != AMVP_SUCCESS) {
            AMVP_LOG_ERR("Hex converstion failure (key)");
            return 0;
        }
    }
    des_tc->iv = calloc(AMVP_SYM_IV_BYTE_MAX, sizeof(char));
    if (!des_tc->iv) { return 0; }
    if (iv) {
        rv = amvp_hexstr_to_bin(iv, des_tc->iv, AMVP_SYM_IV_BYTE_MAX, NULL);
        if (rv != AMVP_SUCCESS) {
    ...
    des_tc->iv_ret = calloc(1, AMVP_SYM_IV_BYTE_MAX + 1);
    des_tc->iv_ret_after = calloc(1, AMVP_SYM_IV_BYTE_MAX + 1);
...
void free_des_tc(AMVP_SYM_CIPHER_TC *des_tc) {
...
Test(APP_DES_HANDLER, bad_keylen) {
    ...
    des_tc = calloc(1, sizeof(AMVP_SYM_CIPHER_TC));

    if (!initialize_des_tc(des_tc, AMVP_TDES_CBC, payload, 32, NULL, 0,
        key, 128, iv, 64, AMVP_SYM_CIPH_DIR_ENCRYPT, AMVP_SYM_TEST_TYPE_AFT)) {
...
Test(APP_DES_HANDLER, bad_iv) {
    ...
    if (!initialize_des_tc(des_tc, AMVP_TDES_CBC, payload, 64, NULL, 0,
        key, 192, iv, 4, AMVP_SYM_CIPH_DIR_ENCRYPT, AMVP_SYM_TEST_TYPE_AFT)) {
        cr_assert_fail("des init tc failure");
    }
    test_case = calloc(1, sizeof(AMVP_TEST_CASE));
    test_case->tc.symmetric = des_tc;
...
Test(APP_DES_HANDLER, missing_dir) {
    ...
    if (!initialize_des_tc(des_tc, AMVP_TDES_ECB, payload, 32, NULL, 0,
        key, 192, iv, 64, 3, AMVP_SYM_TEST_TYPE_AFT)) {
        cr_assert_fail("des init tc failure");
    }
    test_case = calloc(1, sizeof(AMVP_TEST_CASE));
    test_case->tc.symmetric = des_tc;
...
Test(APP_DES_HANDLER, bad_payload_len) {
    ...
    if (!initialize_des_tc(des_tc, AMVP_TDES_ECB, NULL, 0, payload, 8,
        key, 192, iv, 64, AMVP_SYM_CIPH_DIR_DECRYPT, AMVP_SYM_TEST_TYPE_AFT)) {
        cr_assert_fail("des init tc failure");
    }
    test_case = calloc(1, sizeof(AMVP_TEST_CASE));
    test_case->tc.symmetric = des_tc;
 ...
 */
 Test(APP_DES_HANDLER, bad_cipher) {
    ...
    if (!initialize_des_tc(des_tc, AMVP_AES_CTR, payload, 64, NULL, 0,
        key, 192, iv, 64, AMVP_SYM_CIPH_DIR_ENCRYPT, AMVP_SYM_TEST_TYPE_AFT)) {
        cr_assert_fail("des init tc failure");
    }
 ...
 */
 Test(APP_DES_HANDLER, bad_tc_initialization) {
    ...
    if (!initialize_des_tc(des_tc, AMVP_TDES_CFB64, payload, 64, NULL, 0,
        key, 192, iv, 64, AMVP_SYM_CIPH_DIR_ENCRYPT, AMVP_SYM_TEST_TYPE_AFT)) {
        cr_assert_fail("des init tc failure");
    }
 ...
 */
 Test(APP_DES_HANDLER, bad_mct_dir) {
    ...
    if (!initialize_des_tc(des_tc, AMVP_TDES_CFB1, NULL, 0, payload, 64,
        key, 192, iv, 64, 3, AMVP_SYM_TEST_TYPE_MCT)) {
        cr_assert_fail("des init tc failure");
    }
    test_case = calloc(1, sizeof(AMVP_TEST_CASE));
    test_case->tc.symmetric = des_tc;
 ...
 */
 Test(APP_DES_HANDLER, bad_mct_payloadLen) {
    ...
    if (!initialize_des_tc(des_tc, AMVP_TDES_CFB1, NULL, 0, payload, 8,
        key, 192, iv, 64, AMVP_SYM_CIPH_DIR_DECRYPT, AMVP_SYM_TEST_TYPE_MCT)) {
        cr_assert_fail("des init tc failure");
    }
 ...
 */
 Test(APP_DES_HANDLER, bad_mct_iv) {
    ...
    if (!initialize_des_tc(des_tc, AMVP_TDES_CFB1, payload, 32, NULL, 0,
        key, 192, NULL, 0, AMVP_SYM_CIPH_DIR_ENCRYPT, AMVP_SYM_TEST_TYPE_MCT)) {
        cr_assert_fail("des init tc failure");
    }
