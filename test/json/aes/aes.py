...
def convert_to_gcm(s):
    if s[1]["algorithm"] == "AMVP-AES-GCM":
        # Already GCM mode
        return

    s[1]["algorithm"] = "AMVP-AES-GCM"
...
def convert_to_ctr(s):
    if s[1]["algorithm"] == "AMVP-AES-CTR":
        # Already CTR mode
        return

    s[1]["algorithm"] = "AMVP-AES-CTR"
...
def convert_to_ccm(s):
    if s[1]["algorithm"] == "AMVP-AES-CCM":
        # Already CCM mode
        return

    s[1]["algorithm"] = "AMVP-AES-CCM"
...
def gen(j=None):
    ...
    s = copy.deepcopy(j)
    tg, t = ref_testgroup_and_test(s, ENCRYPT)
    tg["payloadLen"] = 131073 # AMVP_SYM_PT_BIT_MAX + 1
    ...
    ##
    # The boolean for "incrementalCounter" is not a boolean for AMVP-AES-CTR
    ##
    s = copy.deepcopy(j)
    convert_to_ctr(s)
    ...
    ##
    # The boolean for "overflowCounter" is not a boolean for AMVP-AES-CTR
    ##
    s = copy.deepcopy(j)
    convert_to_ctr(s)
