...
def convert_to_ctr(s):
    if s[1]["algorithm"] == "AMVP-TDES-CTR":
        # Already CTR mode
        return

    s[1]["algorithm"] = "AMVP-TDES-CTR"
...
def gen(j=None):
    ...
    s = copy.deepcopy(j)
    tg, t = ref_testgroup_and_test(s, ENCRYPT)
    if s[1]["algorithm"] == "AMVP-TDES-ECB":
        # ECB doesn't have iv
        s[1]["algorithm"] = "AMVP-TDES-CBC"
    else:
        del t["iv"]
    ...
    ##
    # The boolean for "incrementalCounter" is not a boolean for AMVP-TDES-CTR
    ##
    s = copy.deepcopy(j)
    convert_to_ctr(s)
    ...
    ##
    # The boolean for "overflowCounter" is not a boolean for AMVP-TDES-CTR
    ##
    s = copy.deepcopy(j)
    convert_to_ctr(s)
