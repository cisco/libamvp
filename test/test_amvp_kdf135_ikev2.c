...
static void setup_fail(void) {
    ...
    rv = amvp_cap_kdf135_ikev2_enable(ctx, &dummy_handler_failure);
    cr_assert(rv == AMVP_SUCCESS);
    rv = amvp_cap_set_prereq(ctx, AMVP_KDF135_IKEV2, AMVP_PREREQ_SHA, cvalue);
    cr_assert(rv == AMVP_SUCCESS);
    rv = amvp_cap_set_prereq(ctx, AMVP_KDF135_IKEV2, AMVP_PREREQ_DRBG, cvalue);
...
Test(KDF135_IKEV2_API, empty_ctx) {
    ...
    rv  = amvp_kdf135_ikev2_kat_handler(ctx, obj);
...
Test(KDF135_IKEV2_API, null_ctx) {
    ...
    rv  = amvp_kdf135_ikev2_kat_handler(NULL, obj);
...
Test(KDF135_IKEV2_API, null_json_obj, .init = setup, .fini = teardown) {
    rv  = amvp_kdf135_ikev2_kat_handler(ctx, NULL);
