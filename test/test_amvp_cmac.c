...
static void setup_fail(void) {
    ...
    rv = amvp_cap_cmac_enable(ctx, AMVP_CMAC_AES, &dummy_handler_failure);
    cr_assert(rv == AMVP_SUCCESS);
    rv = amvp_cap_set_prereq(ctx, AMVP_CMAC_AES, AMVP_PREREQ_AES, cvalue);
    cr_assert(rv == AMVP_SUCCESS);
    
    rv = amvp_cap_cmac_enable(ctx, AMVP_CMAC_TDES, &dummy_handler_failure);
    cr_assert(rv == AMVP_SUCCESS);
    rv = amvp_cap_set_prereq(ctx, AMVP_CMAC_TDES, AMVP_PREREQ_TDES, cvalue);
    cr_assert(rv == AMVP_SUCCESS);
...
Test(CMAC_API, empty_ctx) {
    ...
    rv  = amvp_cmac_kat_handler(ctx, obj);
...
Test(CMAC_API, null_ctx) {
    ...
    rv  = amvp_cmac_kat_handler(NULL, obj);
...
Test(CMAC_API, null_json_obj, .init = setup, .fini = teardown) {
    rv  = amvp_cmac_kat_handler(ctx, NULL);
