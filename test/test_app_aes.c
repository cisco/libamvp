...
AMVP_SYM_CIPHER_TC *aes_tc;
AMVP_RESULT rv;

int initialize_aes_tc(AMVP_SYM_CIPHER_TC *aes_tc, int alg_id, char *pt, 
                      int pt_len, char *ct, int ct_len, char *key, int key_len, char *aad,
                      int aad_len, char *tag, int tag_len, AMVP_SYM_CIPH_IVGEN_SRC ivGen, 
                      AMVP_SYM_CIPH_IVGEN_MODE ivGenMode, char *iv, int iv_len, 
                      AMVP_SYM_CIPH_DIR direction, AMVP_SYM_CIPH_TESTTYPE test_type) {

    aes_tc->ct = calloc(AMVP_SYM_CT_BYTE_MAX, sizeof(char));
    if (!aes_tc->ct) { return 0; }
    if (ct) {
        rv = amvp_hexstr_to_bin(ct, aes_tc->ct, AMVP_SYM_CT_BYTE_MAX, NULL);
        if (rv != AMVP_SUCCESS) {
    ...
    aes_tc->pt = calloc(AMVP_SYM_PT_BYTE_MAX, sizeof(char));
    if (!aes_tc->pt) { return 0; }
    if (pt) {
        rv = amvp_hexstr_to_bin(pt, aes_tc->pt, AMVP_SYM_PT_BYTE_MAX, NULL);
        if (rv != AMVP_SUCCESS) {
    ...
    aes_tc->key = calloc(AMVP_SYM_KEY_MAX_BYTES, sizeof(char));
    if (!aes_tc->key) { return 0; }
    if (key) {
        rv = amvp_hexstr_to_bin(key, aes_tc->key, AMVP_SYM_KEY_MAX_BYTES, NULL);
        if (rv != AMVP_SUCCESS) {
    ...
    aes_tc->aad = calloc(AMVP_SYM_AAD_BYTE_MAX, sizeof(char));
    if (!aes_tc->aad) { return 0; }
    if (aad) {
        rv = amvp_hexstr_to_bin(aad, aes_tc->aad, AMVP_SYM_AAD_BYTE_MAX, NULL);
        if (rv != AMVP_SUCCESS) {
            AMVP_LOG_ERR("Hex converstion failure (aad)");
            return 0;
        }
    }
    

    aes_tc->tag = calloc(AMVP_SYM_TAG_BYTE_MAX, sizeof(char));
    if (!aes_tc->tag) { return 0; }
    if (tag) {
        rv = amvp_hexstr_to_bin(tag, aes_tc->tag, AMVP_SYM_TAG_BYTE_MAX, NULL);
        if (rv != AMVP_SUCCESS) {
            AMVP_LOG_ERR("Hex converstion failure (tag)");
            return 0;
        }
    }
    

    aes_tc->iv = calloc(AMVP_SYM_IV_BYTE_MAX, sizeof(char));
    if (!aes_tc->iv) { return 0; }
    if (iv) {
        rv = amvp_hexstr_to_bin(iv, aes_tc->iv, AMVP_SYM_IV_BYTE_MAX, NULL);
        if (rv != AMVP_SUCCESS) {
...
void free_aes_tc(AMVP_SYM_CIPHER_TC *aes_tc) {
...
Test(APP_AES_AEAD_HANDLER, missing_msg) {
    ...
    aes_tc = calloc(1, sizeof(AMVP_SYM_CIPHER_TC));
    
    if (!initialize_aes_tc(aes_tc, AMVP_AES_GCM, payload, 32, NULL, 0,
        key, 24, aad, 16, tag, 128, AMVP_SYM_CIPH_IVGEN_SRC_INT, 
        AMVP_SYM_CIPH_IVGEN_MODE_821, NULL, 96, AMVP_SYM_CIPH_DIR_ENCRYPT,
...
Test(APP_AES_AEAD_HANDLER, missing_dir) {
    ...
    if (!initialize_aes_tc(aes_tc, AMVP_AES_CCM, payload, 32, NULL, 0,
        key, 128, aad, 16, tag, 0, AMVP_SYM_CIPH_IVGEN_SRC_INT, 
        AMVP_SYM_CIPH_IVGEN_MODE_821, NULL, 96, 3,
        AMVP_SYM_TEST_TYPE_AFT)) {
        cr_assert_fail("aes init tc failure");
    }
    test_case = calloc(1, sizeof(AMVP_TEST_CASE));
 ...
 */
 Test(APP_AES_AEAD_HANDLER, bad_cipher) {
    ...
    if (!initialize_aes_tc(aes_tc, AMVP_HASH_SHA224, payload, 32, NULL, 0,
        key, 128, aad, 0, tag, 128, AMVP_SYM_CIPH_IVGEN_SRC_INT, 
        AMVP_SYM_CIPH_IVGEN_MODE_821, NULL, 96, AMVP_SYM_CIPH_DIR_ENCRYPT,
        AMVP_SYM_TEST_TYPE_AFT)) {
        cr_assert_fail("aes init tc failure");
    }
 ...
 */
 Test(APP_AES_AEAD_HANDLER, bad_taglen) {
    ...
    if (!initialize_aes_tc(aes_tc, AMVP_AES_GCM, NULL, 0, payload, 32,
        key, 128, aad, 0, tag, 64, AMVP_SYM_CIPH_IVGEN_SRC_INT, 
        AMVP_SYM_CIPH_IVGEN_MODE_821, NULL, 96, AMVP_SYM_CIPH_DIR_DECRYPT,
        AMVP_SYM_TEST_TYPE_AFT)) {
        cr_assert_fail("aes init tc failure");
    }
    test_case = calloc(1, sizeof(AMVP_TEST_CASE));
 ...
 */
 Test(APP_AES_AEAD_HANDLER, bad_aadlen) {
    ...
    if (!initialize_aes_tc(aes_tc, AMVP_AES_CCM, payload, 32, NULL, 0,
        key, 128, aad, 7, tag, 64, AMVP_SYM_CIPH_IVGEN_SRC_INT, 
        AMVP_SYM_CIPH_IVGEN_MODE_821, NULL, 96, AMVP_SYM_CIPH_DIR_ENCRYPT,
        AMVP_SYM_TEST_TYPE_AFT)) {
        cr_assert_fail("aes init tc failure");
    }
 ...
 */
 Test(APP_AES_AEAD_HANDLER, bad_gmac) {
    ...
    if (!initialize_aes_tc(aes_tc, AMVP_AES_GMAC, payload, 32, NULL, 0,
        key, 256, aad, 0, tag, 128, AMVP_SYM_CIPH_IVGEN_SRC_INT, 
        AMVP_SYM_CIPH_IVGEN_MODE_821, NULL, 96, AMVP_SYM_CIPH_DIR_ENCRYPT,
        AMVP_SYM_TEST_TYPE_AFT)) {
        cr_assert_fail("aes init tc failure");
    }
...
Test(APP_AES_HANDLER, bad_keylen) {
    ...
    if (!initialize_aes_tc(aes_tc, AMVP_AES_ECB, payload, 32, NULL, 0,
        key, 48, NULL, 0, NULL, 0, AMVP_SYM_CIPH_IVGEN_SRC_INT, 
        AMVP_SYM_CIPH_IVGEN_MODE_821, NULL, 96, AMVP_SYM_CIPH_DIR_ENCRYPT,
        AMVP_SYM_TEST_TYPE_AFT)) {
        cr_assert_fail("aes init tc failure");
    }
...
Test(APP_AES_HANDLER, missing_dir) {
    ...
    if (!initialize_aes_tc(aes_tc, AMVP_AES_CTR, payload, 32, NULL, 0,
        key, 128, NULL, 0, NULL, 0, AMVP_SYM_CIPH_IVGEN_SRC_INT, 
        AMVP_SYM_CIPH_IVGEN_MODE_821, NULL, 96, 3,
        AMVP_SYM_TEST_TYPE_AFT)) {
        cr_assert_fail("aes init tc failure");
    }
 ...
 */
 Test(APP_AES_HANDLER, bad_cipher) {
    ...
    if (!initialize_aes_tc(aes_tc, AMVP_KDF108, NULL, 0, payload, 32,
        key, 128, NULL, 0, NULL, 0, AMVP_SYM_CIPH_IVGEN_SRC_INT, 
        AMVP_SYM_CIPH_IVGEN_MODE_821, NULL, 96, AMVP_SYM_CIPH_DIR_ENCRYPT,
        AMVP_SYM_TEST_TYPE_AFT)) {
        cr_assert_fail("aes init tc failure");
 ...
 */
 Test(APP_AES_HANDLER, bad_mct_payloadLen) {
    ...
    if (!initialize_aes_tc(aes_tc, AMVP_AES_ECB, NULL, 0, payload, 24,
        key, 128, NULL, 0, NULL, 0, AMVP_SYM_CIPH_IVGEN_SRC_INT, 
        AMVP_SYM_CIPH_IVGEN_MODE_821, NULL, 96, AMVP_SYM_CIPH_DIR_DECRYPT,
        AMVP_SYM_TEST_TYPE_MCT)) {
        cr_assert_fail("aes init tc failure");
 ...
 */
 Test(APP_AES_KW_HANDLER, bad_cipher) {
    ...
    aes_tc->kwcipher = AMVP_SYM_KW_CIPHER;
    test_case = calloc(1, sizeof(AMVP_TEST_CASE));
    test_case->tc.symmetric = aes_tc;
 ...
 */
 Test(APP_AES_KW_HANDLER, bad_dir) {
    ...
    if (!initialize_aes_tc(aes_tc, AMVP_AES_KW, payload, 32, NULL, 0,
        key, 128, NULL, 0, NULL, 0, AMVP_SYM_CIPH_IVGEN_SRC_INT, 
        AMVP_SYM_CIPH_IVGEN_MODE_821, NULL, 96, 3,
        AMVP_SYM_TEST_TYPE_AFT)) {
        cr_assert_fail("aes init tc failure");
 ...
 */
 Test(APP_AES_KW_HANDLER, bad_payload) {
    ...
    if (!initialize_aes_tc(aes_tc, AMVP_AES_KW, NULL, 0, payload, 0,
        key, 128, NULL, 0, NULL, 0, AMVP_SYM_CIPH_IVGEN_SRC_INT, 
        AMVP_SYM_CIPH_IVGEN_MODE_821, NULL, 96, AMVP_SYM_CIPH_DIR_ENCRYPT,
        AMVP_SYM_TEST_TYPE_AFT)) {
        cr_assert_fail("aes init tc failure");
 ...
 */
 Test(APP_AES_KW_HANDLER, bad_key) {
    ...
    if (!initialize_aes_tc(aes_tc, AMVP_AES_KW, payload, 32, payload, 32,
        key, 128, NULL, 0, NULL, 0, AMVP_SYM_CIPH_IVGEN_SRC_INT, 
        AMVP_SYM_CIPH_IVGEN_MODE_821, NULL, 96, AMVP_SYM_CIPH_DIR_DECRYPT,
        AMVP_SYM_TEST_TYPE_AFT)) {
        cr_assert_fail("aes init tc failure");
