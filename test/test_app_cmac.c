...
AMVP_CMAC_TC *cmac_tc;
AMVP_RESULT rv;

int initialize_cmac_tc(AMVP_CMAC_TC *cmac_tc,
                       int alg_id, char *mac,
                       char *msg, int msg_len,
                       char *key, int key_len,
                       char *key2, char *key3,
                       int direction_verify, int corrupt) {
    memset(cmac_tc, 0x0, sizeof(AMVP_CMAC_TC));
    
    if (!corrupt) {
        cmac_tc->mac = calloc(1, AMVP_CMAC_MACLEN_MAX);
        if (!cmac_tc->mac) { return -1; }
    }
    if (direction_verify) {
        cmac_tc->verify = 1;
    
        rv = amvp_hexstr_to_bin(mac, cmac_tc->mac, AMVP_CMAC_MACLEN_MAX, (int *)&(cmac_tc->mac_len));
        if (rv != AMVP_SUCCESS) {
    ...
    if (msg) {
        cmac_tc->msg = calloc(1, AMVP_CMAC_MSGLEN_MAX_STR);
        if (!cmac_tc->msg) { return -1; }
        rv = amvp_hexstr_to_bin(msg, cmac_tc->msg, AMVP_CMAC_MSGLEN_MAX_STR, NULL);
        if (rv != AMVP_SUCCESS) {
    ...
    if (alg_id == AMVP_CMAC_AES) {
        if (key) {
            cmac_tc->key = calloc(1, AMVP_CMAC_KEY_MAX);
            if (!cmac_tc->key) { return AMVP_MALLOC_FAIL; }
            rv = amvp_hexstr_to_bin(key, cmac_tc->key, AMVP_CMAC_KEY_MAX, (int *) &(cmac_tc->key_len));
            if (rv != AMVP_SUCCESS) {
        ...
        if (key) {
            cmac_tc->key = calloc(1, AMVP_CMAC_KEY_MAX);
            if (!cmac_tc->key) { return AMVP_MALLOC_FAIL; }
            rv = amvp_hexstr_to_bin(key, cmac_tc->key, AMVP_CMAC_KEY_MAX, NULL);
            if (rv != AMVP_SUCCESS) {
        ...
        if (key2) {
            cmac_tc->key2 = calloc(1, AMVP_CMAC_KEY_MAX);
            if (!cmac_tc->key2) { return AMVP_MALLOC_FAIL; }
            rv = amvp_hexstr_to_bin(key2, cmac_tc->key2, AMVP_CMAC_KEY_MAX, NULL);
            if (rv != AMVP_SUCCESS) {
        ...
        if (key3) {
            cmac_tc->key3 = calloc(1, AMVP_CMAC_KEY_MAX);
            if (!cmac_tc->key3) { return AMVP_MALLOC_FAIL; }
            rv = amvp_hexstr_to_bin(key3, cmac_tc->key3, AMVP_CMAC_KEY_MAX, NULL);
            if (rv != AMVP_SUCCESS) {
...
void free_cmac_tc(AMVP_CMAC_TC *cmac_tc) {
...
Test(APP_CMAC_HANDLER, missing_msg) {
    char *msg = NULL;
    char *key = "aaaa";
    cmac_tc = calloc(1, sizeof(AMVP_CMAC_TC));
    
    if (!initialize_cmac_tc(cmac_tc, AMVP_CMAC_AES, NULL, msg, 8, key, 16, NULL, NULL, 0, 0)) {
...
Test(APP_CMAC_HANDLER, missing_keys_tdes) {
    ...
    if (!initialize_cmac_tc(cmac_tc, AMVP_CMAC_TDES, NULL, msg, 8, key, 16, NULL, NULL, 0, 0)) {
        cr_assert_fail("hash init tc failure");
    }
    test_case = calloc(1, sizeof(AMVP_TEST_CASE));
    test_case->tc.cmac = cmac_tc;
...
Test(APP_CMAC_HANDLER, disposition_mem_not_allocated) {
    ...
    if (!initialize_cmac_tc(cmac_tc, AMVP_CMAC_AES, NULL, msg, 8, key, 16, NULL, NULL, 0, 1)) {
        cr_assert_fail("hash init tc failure");
    }
    test_case = calloc(1, sizeof(AMVP_TEST_CASE));
    test_case->tc.cmac = cmac_tc;
