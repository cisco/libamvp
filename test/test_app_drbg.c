...
AMVP_DRBG_TC *drbg_tc;
AMVP_RESULT rv;

void free_drbg_tc(AMVP_DRBG_TC *stc) {
...
int initialize_drbg_tc(AMVP_DRBG_TC *drbg_tc, int alg_id, int mode_id,
    ...
    if (!corrupt) {
        drbg_tc->drb = calloc(AMVP_DRB_BYTE_MAX, sizeof(unsigned char));
        if (!drbg_tc->drb) { goto err; }
    }
    if (ad_in_0) {
        drbg_tc->additional_input_0 = calloc(AMVP_DRBG_ADDI_IN_BYTE_MAX, sizeof(unsigned char));
        if (!drbg_tc->additional_input_0) { goto err; }
    }
    if (ad_in_1) {
        drbg_tc->additional_input_1 = calloc(AMVP_DRBG_ADDI_IN_BYTE_MAX, sizeof(unsigned char));
        if (!drbg_tc->additional_input_1) { goto err; }
    }
    if (ad_in_2) {
        drbg_tc->additional_input_2 = calloc(AMVP_DRBG_ADDI_IN_BYTE_MAX, sizeof(unsigned char));
        if (!drbg_tc->additional_input_2) { goto err; }
    }
    if (ent) {
        drbg_tc->entropy = calloc(AMVP_DRBG_ENTPY_IN_BYTE_MAX, sizeof(unsigned char));
        if (!drbg_tc->entropy) { goto err; }
    }
    if (ent_in_0) {
        drbg_tc->entropy_input_pr_0 = calloc(AMVP_DRBG_ENTPY_IN_BYTE_MAX, sizeof(unsigned char));
        if (!drbg_tc->entropy_input_pr_0) { goto err; }
    }
    if (ent_in_1) {
        drbg_tc->entropy_input_pr_1 = calloc(AMVP_DRBG_ENTPY_IN_BYTE_MAX, sizeof(unsigned char));
        if (!drbg_tc->entropy_input_pr_1) { goto err; }
    }
    if (ent_in_2) {
        drbg_tc->entropy_input_pr_2 = calloc(AMVP_DRBG_ENTPY_IN_BYTE_MAX, sizeof(unsigned char));
        if (!drbg_tc->entropy_input_pr_2) { goto err; }
    }
    if (nonce) {
        drbg_tc->nonce = calloc(AMVP_DRBG_NONCE_BYTE_MAX, sizeof(unsigned char));
        if (!drbg_tc->nonce) { goto err; }
    }
    if (perso_string) {
        drbg_tc->perso_string = calloc(AMVP_DRBG_PER_SO_BYTE_MAX, sizeof(unsigned char));
        if (!drbg_tc->perso_string) { goto err; }
    }

    if (ad_in_0) {
        rv = amvp_hexstr_to_bin(ad_in_0, drbg_tc->additional_input_0,
                                AMVP_DRBG_ADDI_IN_BYTE_MAX, NULL);
        if (rv != AMVP_SUCCESS) {
            AMVP_LOG_ERR("Hex conversion failure (ad_in_0)");
            goto err;
        }
    }

    if (ent_in_0) {
        rv = amvp_hexstr_to_bin(ent_in_0, drbg_tc->entropy_input_pr_0,
                                AMVP_DRBG_ENTPY_IN_BYTE_MAX, NULL);
        if (rv != AMVP_SUCCESS) {
            AMVP_LOG_ERR("Hex conversion failure (ent_in_0)");
            goto err;
        }
    }

    if (ad_in_1) {
        rv = amvp_hexstr_to_bin(ad_in_1, drbg_tc->additional_input_1,
                                AMVP_DRBG_ADDI_IN_BYTE_MAX, NULL);
        if (rv != AMVP_SUCCESS) {
            AMVP_LOG_ERR("Hex conversion failure (ad_in_1)");
            goto err;
        }
    }

    if (ent_in_1) {
        rv = amvp_hexstr_to_bin(ent_in_1, drbg_tc->entropy_input_pr_1,
                                AMVP_DRBG_ENTPY_IN_BYTE_MAX, NULL);
        if (rv != AMVP_SUCCESS) {
            AMVP_LOG_ERR("Hex conversion failure (ent_in_1)");
            goto err;
        }
    }

    if (ad_in_2) {
        rv = amvp_hexstr_to_bin(ad_in_2, drbg_tc->additional_input_2,
                                AMVP_DRBG_ADDI_IN_BYTE_MAX, NULL);
        if (rv != AMVP_SUCCESS) {
    ...
    if (ent_in_2) {
        rv = amvp_hexstr_to_bin(ent_in_2, drbg_tc->entropy_input_pr_2,
                                AMVP_DRBG_ENTPY_IN_BYTE_MAX, NULL);
        if (rv != AMVP_SUCCESS) {
    ...
    if (ent) {
        rv = amvp_hexstr_to_bin(ent, drbg_tc->entropy,
                                AMVP_DRBG_ENTPY_IN_BYTE_MAX, NULL);
        if (rv != AMVP_SUCCESS) {
            AMVP_LOG_ERR("Hex conversion failure (ent)");
            goto err;
        }
    }

    if (perso_string) {
        rv = amvp_hexstr_to_bin(perso_string, drbg_tc->perso_string,
    ...
    if (nonce) {
        rv = amvp_hexstr_to_bin(nonce, drbg_tc->nonce,
...
Test(APP_DRBG_HANDLER, bad_cipher) {
    ...
    drbg_tc = calloc(1, sizeof(AMVP_DRBG_TC));

    if (!initialize_drbg_tc(drbg_tc, AMVP_HASH_SHAKE_128, AMVP_DRBG_TDES,
            ad0, ent0, ad1,
            ent1, ad2, ent2,
...
Test(APP_DRBG_HANDLER, bad_mode) {
    ...
    drbg_tc = calloc(1, sizeof(AMVP_DRBG_TC));
    if (!initialize_drbg_tc(drbg_tc, AMVP_HASHDRBG, AMVP_DRBG_TDES,
            ad0, ent0, ad1,
            ent1, ad2, ent2,
            64, 64, //pr_1 len, pr_2 len
            perso, ent, nonce,
    ...
    drbg_tc = calloc(1, sizeof(AMVP_DRBG_TC));
    if (!initialize_drbg_tc(drbg_tc, AMVP_HMACDRBG, AMVP_DRBG_AES_128,
            ad0, ent0, ad1,
            ent1, ad2, ent2,
            64, 64, //pr_1 len, pr_2 len
            perso, ent, nonce,
    ...
    drbg_tc = calloc(1, sizeof(AMVP_DRBG_TC));
    if (!initialize_drbg_tc(drbg_tc, AMVP_CTRDRBG, AMVP_DRBG_SHA_256,
            ad0, ent0, ad1,
            ent1, ad2, ent2,
            64, 64, //pr_1 len, pr_2 len
            perso, ent, nonce,
...
Test(APP_DRBG_HANDLER, bad_alloc_out) {
    ...
    if (!initialize_drbg_tc(drbg_tc, AMVP_HASHDRBG, AMVP_DRBG_SHA_256,
            ad0, ent0, ad1,
            ent1, ad2, ent2,
...
Test(APP_DRBG_HANDLER, missing_entropy) {
    ...
    if (!initialize_drbg_tc(drbg_tc, AMVP_CTRDRBG, AMVP_DRBG_AES_256,
            ad0, ent0, ad1,
            ent1, ad2, ent2,
