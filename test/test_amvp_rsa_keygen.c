...
static void setup_fail(void) {
    ...
    rv = amvp_cap_rsa_keygen_enable(ctx, AMVP_RSA_KEYGEN, &dummy_handler_failure);
    cr_assert(rv == AMVP_SUCCESS);
    rv = amvp_cap_set_prereq(ctx, AMVP_RSA_KEYGEN, AMVP_PREREQ_SHA, cvalue);
    cr_assert(rv == AMVP_SUCCESS);
    rv = amvp_cap_set_prereq(ctx, AMVP_RSA_KEYGEN, AMVP_PREREQ_DRBG, cvalue);
...
Test(RSA_KEYGEN_API, empty_ctx) {
    ...
    rv  = amvp_rsa_keygen_kat_handler(ctx, obj);
...
Test(RSA_KEYGEN_API, null_ctx) {
    ...
    rv  = amvp_rsa_keygen_kat_handler(NULL, obj);
...
Test(RSA_KEYGEN_API, null_json_obj, .init = setup, .fini = teardown) {
    rv  = amvp_rsa_keygen_kat_handler(ctx, NULL);
