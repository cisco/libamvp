...
static void setup_siggen(void) {
    ...
    rv = amvp_cap_set_prereq(ctx, AMVP_RSA_SIGGEN, AMVP_PREREQ_SHA, cvalue);
    cr_assert(rv == AMVP_SUCCESS);
...
static void setup_siggen_fail(void) {
    ...
    rv = amvp_cap_rsa_sig_enable(ctx, AMVP_RSA_SIGGEN, &dummy_handler_failure);
    cr_assert(rv == AMVP_SUCCESS);
    rv = amvp_cap_set_prereq(ctx, AMVP_RSA_SIGGEN, AMVP_PREREQ_SHA, cvalue);
    cr_assert(rv == AMVP_SUCCESS);
    rv = amvp_cap_rsa_siggen_set_type(ctx, AMVP_RSA_SIG_TYPE_X931);
...
static void setup_sigver(void) {
    ...
    rv = amvp_cap_set_prereq(ctx, AMVP_RSA_SIGVER, AMVP_PREREQ_SHA, cvalue);
    cr_assert(rv == AMVP_SUCCESS);
...
static void setup_sigver_fail(void) {
    ...
    rv = amvp_cap_rsa_sig_enable(ctx, AMVP_RSA_SIGVER, &dummy_handler_failure);
    cr_assert(rv == AMVP_SUCCESS);
    rv = amvp_cap_rsa_sigver_set_parm(ctx, AMVP_RSA_PARM_PUB_EXP_MODE, AMVP_RSA_PUB_EXP_MODE_FIXED);
    cr_assert(rv == AMVP_SUCCESS);
    rv = amvp_cap_rsa_sigver_set_exponent(ctx, AMVP_RSA_PARM_FIXED_PUB_EXP_VAL, expo_str);
...
Test(RSA_SIGGEN_API, empty_ctx) {
    ...
    rv  = amvp_rsa_siggen_kat_handler(ctx, obj);
...
Test(RSA_SIGGEN_API, null_ctx) {
    ...
    rv  = amvp_rsa_siggen_kat_handler(NULL, obj);
...
Test(RSA_SIGGEN_API, null_json_obj, .init = setup_siggen, .fini = teardown) {
    rv  = amvp_rsa_siggen_kat_handler(ctx, NULL);
...
Test(RSA_SIGVER_API, empty_ctx) {
    ...
    rv  = amvp_rsa_sigver_kat_handler(ctx, obj);
    cr_assert(rv == AMVP_UNSUPPORTED_OP);
    json_value_free(val);
...
Test(RSA_SIGVER_API, null_ctx) {
    ...
    /* Test with NULL JSON object */
    rv  = amvp_rsa_sigver_kat_handler(NULL, obj);
    cr_assert(rv == AMVP_NO_CTX);
    json_value_free(val);
}
...
Test(RSA_SIGVER_API, null_json_obj, .init = setup_sigver, .fini = teardown) {
    rv  = amvp_rsa_sigver_kat_handler(ctx, NULL);
    cr_assert(rv == AMVP_MALFORMED_JSON);
}
