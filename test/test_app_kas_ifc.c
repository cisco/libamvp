...
AMVP_KAS_IFC_TC *kas_ifc_tc;
AMVP_RESULT rv;

void free_kas_ifc_tc(AMVP_KAS_IFC_TC *stc) {
    if (stc->z) free(stc->z);
...
int initialize_kas_ifc_tc(AMVP_KAS_IFC_TC *stc,
    ...
    if (stc->kas_role == AMVP_KAS_IFC_RESPONDER) {

if (ct) {
        stc->ct = calloc(1, AMVP_KAS_IFC_BYTE_MAX);
        if (!stc->ct) { return AMVP_MALLOC_FAIL; }
        rv = amvp_hexstr_to_bin(ct, stc->ct, AMVP_KAS_IFC_BYTE_MAX, &(stc->ct_len));
        if (rv != AMVP_SUCCESS) {
...
if (z) {
        stc->z = calloc(1, AMVP_KAS_IFC_BYTE_MAX);
        if (!stc->z) { return AMVP_MALLOC_FAIL; }
        rv = amvp_hexstr_to_bin(z, stc->z, AMVP_KAS_IFC_BYTE_MAX, &(stc->zlen));
        if (rv != AMVP_SUCCESS) {
...
if (hashz) {
        stc->hashz = calloc(1, AMVP_KAS_IFC_BYTE_MAX);
        if (!stc->hashz) { return AMVP_MALLOC_FAIL; }
        rv = amvp_hexstr_to_bin(hashz, stc->hashz, AMVP_KAS_IFC_BYTE_MAX, &(stc->hashzlen));
        if (rv != AMVP_SUCCESS) {
            AMVP_LOG_ERR("Hex conversion failure (hashz)");
            goto err;
        }
}
...
if (c) {
            stc->c = calloc(1, AMVP_KAS_IFC_BYTE_MAX);
            if (!stc->c) { return AMVP_MALLOC_FAIL; }
            rv = amvp_hexstr_to_bin(c, stc->c, AMVP_KAS_IFC_BYTE_MAX, &(stc->clen));
            if (rv != AMVP_SUCCESS) {
                AMVP_LOG_ERR("Hex conversion failure (c)");
                goto err;
            }
}
        }
    ...
    if (corrupt != 1) {
        stc->pt = calloc(1, AMVP_KAS_IFC_BYTE_MAX);
        if (!stc->pt) { return AMVP_MALLOC_FAIL; }
    }
    if (corrupt != 2) {

        stc->chash = calloc(1, AMVP_KAS_IFC_BYTE_MAX);
...
Test(APP_KAS_IFC_HANDLER, invalid_hash_alg) {
    ...
    char *z = "aa";
    char *hashz = "aa";
    AMVP_KAS_IFC_KEYGEN key_gen = AMVP_KAS_IFC_RSAKPG1_BASIC;
    AMVP_KAS_IFC_TEST_TYPE test_type = AMVP_KAS_IFC_TT_AFT;
    AMVP_KAS_IFC_ROLES role = AMVP_KAS_IFC_INITIATOR;
    
    kas_ifc_tc = calloc(1, sizeof(AMVP_KAS_IFC_TC));
...
Test(APP_KAS_IFC_HANDLER, missing_p) {
    ...
    AMVP_KAS_IFC_KEYGEN key_gen = AMVP_KAS_IFC_RSAKPG1_BASIC;
    AMVP_KAS_IFC_TEST_TYPE test_type = AMVP_KAS_IFC_TT_AFT;
    AMVP_KAS_IFC_ROLES role = AMVP_KAS_IFC_RESPONDER;
...
Test(APP_KAS_IFC_HANDLER, missing_chash2) {
    ...
    char *hashz = "aa";
    AMVP_KAS_IFC_KEYGEN key_gen = AMVP_KAS_IFC_RSAKPG1_BASIC;
    AMVP_KAS_IFC_TEST_TYPE test_type = AMVP_KAS_IFC_TT_VAL;
    AMVP_KAS_IFC_ROLES role = AMVP_KAS_IFC_RESPONDER;
...
Test(APP_KAS_IFC_HANDLER, missing_g) {
    ...
    if (!initialize_kas_ifc_tc(kas_ifc_tc, hash_alg, AMVP_KAS_IFC_TT_VAL, p, q, g,
...
Test(APP_KAS_IFC_HANDLER, missing_z) {
    ...
    if (!initialize_kas_ifc_tc(kas_ifc_tc, hash_alg, AMVP_KAS_ECC_TT_AFT, p, q, g,
            ps, epri, epui, z, corrupt)) {
        cr_assert_fail("kas ifc init tc failure");
    }
    test_case = calloc(1, sizeof(AMVP_TEST_CASE));
