...
static void setup(void) {
    ...
    rv = amvp_cap_pbkdf_enable(ctx, &dummy_handler_success);
    cr_assert(rv == AMVP_SUCCESS);
    rv = amvp_cap_set_prereq(ctx, AMVP_PBKDF, AMVP_PREREQ_HMAC, cvalue);
    cr_assert(rv == AMVP_SUCCESS);
    ...
    cr_assert(rv == AMVP_SUCCESS);
    rv = amvp_cap_pbkdf_set_parm(ctx, AMVP_PBKDF_HMAC_ALG, AMVP_SHA3_224);
    cr_assert(rv == AMVP_SUCCESS);
    rv = amvp_cap_pbkdf_set_parm(ctx, AMVP_PBKDF_HMAC_ALG, AMVP_SHA3_256);
    cr_assert(rv == AMVP_SUCCESS);
    rv = amvp_cap_pbkdf_set_parm(ctx, AMVP_PBKDF_HMAC_ALG, AMVP_SHA3_384);
    cr_assert(rv == AMVP_SUCCESS);
    rv = amvp_cap_pbkdf_set_parm(ctx, AMVP_PBKDF_HMAC_ALG, AMVP_SHA3_512);
    cr_assert(rv == AMVP_SUCCESS);
    rv = amvp_cap_pbkdf_set_domain(ctx, AMVP_PBKDF_ITERATION_COUNT, 10, 1000, 1);
    cr_assert(rv == AMVP_SUCCESS);
    rv = amvp_cap_pbkdf_set_domain(ctx, AMVP_PBKDF_KEY_LEN, 112, 4096, 8);
    cr_assert(rv == AMVP_SUCCESS);
    rv = amvp_cap_pbkdf_set_domain(ctx, AMVP_PBKDF_PASSWORD_LEN, 8, 128, 1);
    cr_assert(rv == AMVP_SUCCESS);
...
Test(PBKDF_API, empty_ctx) {
    ...
    rv  = amvp_pbkdf_kat_handler(ctx, obj);
...
Test(PBKDF_API, null_ctx) {
    ...
    rv  = amvp_pbkdf_kat_handler(NULL, obj);
...
Test(PBKDF_API, null_json_obj, .init = setup, .fini = teardown) {
    rv  = amvp_pbkdf_kat_handler(ctx, NULL);
