...
AMVP_HMAC_TC *hmac_tc;
AMVP_RESULT rv;

int initialize_hmac_tc(AMVP_HMAC_TC *hmac_tc, int alg_id, char *msg, int msg_len, char *key, int key_len, int corrupt) {
    memset(hmac_tc, 0x0, sizeof(AMVP_HMAC_TC));
    
    if (!corrupt) {
        hmac_tc->mac = calloc(1, AMVP_HMAC_MAC_BYTE_MAX);
        if (!hmac_tc->mac) { return -1; }
    }
    
    if (msg) {
        hmac_tc->msg = calloc(1, AMVP_HMAC_MSG_MAX);
        if (!hmac_tc->msg) { return -1; }
        rv = amvp_hexstr_to_bin(msg, hmac_tc->msg, AMVP_HMAC_MSG_MAX, NULL);
        if (rv != AMVP_SUCCESS) {
    ...
    if (key) {
        hmac_tc->key = calloc(1, AMVP_HMAC_KEY_BYTE_MAX);
        if (!hmac_tc->key) { return -1; }
        rv = amvp_hexstr_to_bin(msg, hmac_tc->key, AMVP_HMAC_KEY_BYTE_MAX, NULL);
        if (rv != AMVP_SUCCESS) {
            AMVP_LOG_ERR("Hex converstion failure (msg)");
            return -1;
        }
...
void free_hmac_tc(AMVP_HMAC_TC *hmac_tc) {
...
Test(APP_HMAC_HANDLER, missing_msg) {
    char *msg = NULL;
    char *key = "aaaa";
    hmac_tc = calloc(1, sizeof(AMVP_HMAC_TC));
    
    if (!initialize_hmac_tc(hmac_tc, AMVP_HMAC_SHA1, msg, 8, key, 16, 0)) {
...
Test(APP_HMAC_HANDLER, disposition_mem_not_allocated) {
    ...
    if (!initialize_hmac_tc(hmac_tc, AMVP_HMAC_SHA1, msg, 22688, key, 16, 1)) {
        cr_assert_fail("hash init tc failure");
    }
    test_case = calloc(1, sizeof(AMVP_TEST_CASE));
    test_case->tc.hmac = hmac_tc;
