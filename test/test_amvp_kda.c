...
static void setup(void) {
    ...
    rv = amvp_cap_set_prereq(ctx, AMVP_KDA_HKDF, AMVP_PREREQ_HMAC, cvalue);
    cr_assert(rv == AMVP_SUCCESS);
    ...
    cr_assert(rv == AMVP_SUCCESS);
    rv = amvp_cap_set_prereq(ctx, AMVP_KDA_ONESTEP, AMVP_PREREQ_HMAC, cvalue);
    cr_assert(rv == AMVP_SUCCESS);
...
Test(KDA_API, empty_ctx) {
    ...
    rv  = amvp_kda_hkdf_kat_handler(ctx, obj);
    ...
    rv = amvp_kda_onestep_kat_handler(ctx, obj);
    cr_assert(rv == AMVP_UNSUPPORTED_OP);
    json_value_free(val);
...
Test(KDA_API, null_ctx) {
    ...
    rv  = amvp_kda_hkdf_kat_handler(NULL, obj);
    ...
    rv  = amvp_kda_onestep_kat_handler(NULL, obj);
    cr_assert(rv == AMVP_NO_CTX);
    json_value_free(val);
}
...
Test(KDA_API, null_json_obj, .init = setup, .fini = teardown) {
    rv  = amvp_kda_hkdf_kat_handler(ctx, NULL);
    cr_assert(rv == AMVP_MALFORMED_JSON);
    rv = AMVP_SUCCESS;
    rv  = amvp_kda_onestep_kat_handler(ctx, NULL);
    cr_assert(rv == AMVP_MALFORMED_JSON);
...
kdfConfiguration object. Libamvp parses from the 
