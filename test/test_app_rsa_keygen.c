...
AMVP_RSA_KEYGEN_TC *rsa_tc;
AMVP_RESULT rv;

void free_rsa_keygen_tc(AMVP_RSA_KEYGEN_TC *stc) {
    if (stc->e) { free(stc->e); }
...
int initialize_rsa_tc(AMVP_RSA_KEYGEN_TC *stc,
...
Test(APP_RSA_KEYGEN_HANDLER, invalid_hash_alg) {
    ...
    int pub_exp_mode = AMVP_RSA_PUB_EXP_MODE_FIXED;
    int modulo = 2048;
    int key_format = AMVP_RSA_KEY_FORMAT_STANDARD;
    int prime_test = AMVP_RSA_PRIME_TEST_TBLC2;
    int rand_pq = AMVP_RSA_KEYGEN_B33;
    int seed_len = 8;
    char *e = "aa";
    char *seed = "aa";
    
    rsa_tc = calloc(1, sizeof(AMVP_RSA_KEYGEN_TC));
...
Test(APP_RSA_KEYGEN_HANDLER, invalid_key_format) {
    ...
    rsa_tc = calloc(1, sizeof(AMVP_RSA_KEYGEN_TC));
    if (!initialize_rsa_tc(rsa_tc, info_gen_by_server, AMVP_SHA384, key_format,
    pub_exp_mode, modulo, prime_test, rand_pq, e, seed, seed_len,
    bitlen1, bitlen2, bitlen3, bitlen4, corrupt)) {
        cr_assert_fail("rsa keygen init tc failure");
    }
...
Test(APP_RSA_KEYGEN_HANDLER, invalid_pub_exp_mode) {
    ...
    int pub_exp_mode = 0;
    int modulo = 2048;
    int key_format = AMVP_RSA_KEY_FORMAT_CRT;
    int prime_test = AMVP_RSA_PRIME_TEST_TBLC2;
    int rand_pq = AMVP_RSA_KEYGEN_B33;
    int seed_len = 8;
    char *e = "aa";
...
Test(APP_RSA_KEYGEN_HANDLER, invalid_rand_pq) {
    ...
    int modulo = 3072;
    int key_format = AMVP_RSA_KEY_FORMAT_STANDARD;
    int prime_test = AMVP_RSA_PRIME_TEST_TBLC3;
    int rand_pq = 0;
    int seed_len = 8;
    char *e = "aa";
    char *seed = "aa";
