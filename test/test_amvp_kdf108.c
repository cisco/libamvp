...
static void fail_setup(void) {
    ...
    rv = amvp_cap_kdf108_enable(ctx, &dummy_handler_failure);
    cr_assert(rv == AMVP_SUCCESS);
    rv = amvp_cap_set_prereq(ctx, AMVP_KDF108, AMVP_PREREQ_HMAC, cvalue);
    cr_assert(rv == AMVP_SUCCESS);
    rv = amvp_cap_kdf108_set_domain(ctx, AMVP_KDF108_MODE_COUNTER, AMVP_KDF108_SUPPORTED_LEN, 8, 384, 8);
...
Test(KDF108_API, empty_ctx) {
    ...
    rv  = amvp_kdf108_kat_handler(ctx, obj);
...
Test(KDF108_API, null_ctx) {
    ...
    rv  = amvp_kdf108_kat_handler(NULL, obj);
...
Test(KDF108_API, null_json_obj, .init = setup, .fini = teardown) {
    rv  = amvp_kdf108_kat_handler(ctx, NULL);
