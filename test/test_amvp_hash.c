...
static void fail_setup(void) {
    ...
    rv = amvp_cap_hash_enable(ctx, AMVP_HASH_SHA256, &dummy_handler_failure);
    cr_assert(rv == AMVP_SUCCESS);
    rv = amvp_cap_hash_set_domain(ctx, AMVP_HASH_SHA256, AMVP_HASH_MESSAGE_LEN, 0, 65528, 8);
    cr_assert(rv == AMVP_SUCCESS);
}
...
Test(HASH_API, empty_ctx) {
    ...
    rv  = amvp_hash_kat_handler(ctx, obj);
...
Test(HASH_API, null_ctx) {
    ...
    rv  = amvp_hash_kat_handler(NULL, obj);
...
Test(HASH_API, null_json_obj, .init = setup, .fini = teardown) {
    rv  = amvp_hash_kat_handler(ctx, NULL);
