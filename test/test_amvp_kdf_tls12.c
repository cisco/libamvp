...
static void setup(void) {
    ...
    rv = amvp_cap_kdf_tls12_enable(ctx, &dummy_handler_success);
    cr_assert(rv == AMVP_SUCCESS);
    rv = amvp_cap_set_prereq(ctx, AMVP_KDF_TLS12, AMVP_PREREQ_SHA, cvalue);
    cr_assert(rv == AMVP_SUCCESS);
    rv = amvp_cap_set_prereq(ctx, AMVP_KDF_TLS12, AMVP_PREREQ_HMAC, cvalue);
    cr_assert(rv == AMVP_SUCCESS);
...
static void setup_fail(void) {
    ...
    rv = amvp_cap_kdf_tls12_enable(ctx, &dummy_handler_failure);
    cr_assert(rv == AMVP_SUCCESS);
    rv = amvp_cap_set_prereq(ctx, AMVP_KDF_TLS12, AMVP_PREREQ_SHA, cvalue);
    cr_assert(rv == AMVP_SUCCESS);
    rv = amvp_cap_set_prereq(ctx, AMVP_KDF_TLS12, AMVP_PREREQ_HMAC, cvalue);
...
Test(KDF_TLS12_API, empty_ctx) {
    ...
    rv  = amvp_kdf_tls12_kat_handler(ctx, obj);
...
Test(KDF_TLS12_API, null_ctx) {
    ...
    rv  = amvp_kdf_tls12_kat_handler(NULL, obj);
...
Test(KDF_TLS12_API, null_json_obj, .init = setup, .fini = teardown) {
    rv  = amvp_kdf_tls12_kat_handler(ctx, NULL);
