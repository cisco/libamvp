...
class Resource:
    def __init__(self, data=None):
        ...
        self.to_send = [{"amvVersion": AMVP_VERSION}, data]
...
def login(refresh=False):
    ...
    if refresh:
        j = [{"amvVersion": AMVP_VERSION}, {"accessToken": ACCESS_TOKEN}]
    else:
        if TOTP_SEED:
            j = [{"amvVersion": AMVP_VERSION}, {"password": totp()}]
        else:
            j = [{"amvVersion": AMVP_VERSION}]
